...
public abstract partial class ScriptableSkill : ScriptableObject
{
    ...
    bool CheckWeapon(EntityOLD caster)
    {
    ...
    //  if we have ammo, etc.)
    public virtual bool CheckSelf(EntityOLD caster, int skillLevel)
    {
    ...
    //      while casting it
    public abstract bool CheckTarget(EntityOLD caster);
    ...
    public abstract bool CheckDistance(EntityOLD caster, int skillLevel, out Vector3 destination);

    // 4. apply skill: deal damage, heal, launch projectiles, etc.
    // (has corrected target already)
    public abstract void Apply(EntityOLD caster, int skillLevel);

    // events for client sided effects /////////////////////////////////////////
    // [Client]
    public virtual void OnCastStarted(EntityOLD caster)
    {
    ...
    // [Client]
    public virtual void OnCastFinished(EntityOLD caster) {}
