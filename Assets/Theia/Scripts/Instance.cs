...
public class Instance : MonoBehaviour
{
    ...
    HashSet<PlayerOLD> FindAllPlayersInInstanceBounds()
    {
        ...
        for (int i = 0; i < hits; ++i)
        {
            ...
            PlayerOLD player = co.GetComponentInParent<PlayerOLD>();
            if (player != null)
                result.Add(player);
        }
        return result;
        ...
        if (partyId > 0)
        {
            ...
            if (NetworkServer.active)
            {
                ...
                if (NetworkTime.time >= nextPartyCheckTime)
                {
                    ...
                    //   instance
                    HashSet<PlayerOLD> playersInInstanceBounds = FindAllPlayersInInstanceBounds();

                    int playersRemaining = 0;
                    foreach (PlayerOLD player in playersInInstanceBounds)
                    {
            ...
            //    client.
            else if (PlayerOLD.localPlayer != null)
            {
                // has the player entered the instance yet?
                if (bounds.Contains(PlayerOLD.localPlayer.transform.position))
                {
