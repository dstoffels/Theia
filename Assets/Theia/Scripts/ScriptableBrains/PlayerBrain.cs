...
public class PlayerBrain : CommonBrain
{
    ...
    // events //////////////////////////////////////////////////////////////////
    public bool EventCancelAction(PlayerOLD player)
    {
    ...
    public bool EventCraftingStarted(PlayerOLD player)
    {
    ...
    public bool EventCraftingDone(PlayerOLD player) =>
        player.state == "CRAFTING" && NetworkTime.time > player.crafting.endTime;

    public bool EventRespawn(PlayerOLD player)
    {
    ...
    public bool EventTradeDone(PlayerOLD player) =>
        player.state == "TRADING" && player.trading.requestFrom == "";

    // did someone request a trade? and did we request a trade with him too?
    public bool EventTradeStarted(PlayerOLD player)
    {
        PlayerOLD other = player.trading.FindPlayerFromInvitation();
        return other != null && other.trading.requestFrom == player.name;
    }

    // states //////////////////////////////////////////////////////////////////
    string UpdateServer_IDLE(PlayerOLD player)
    {
    ...
    string UpdateServer_MOVING(PlayerOLD player)
    {
        // events sorted by priority (e.g. target doesn't matter if we died)
        if (EventDied(player))
        {
    ...
    void UseNextTargetIfAny(PlayerOLD player)
    {
    ...
    string UpdateServer_CASTING(PlayerOLD player)
    {
    ...
    string UpdateServer_STUNNED(PlayerOLD player)
    {
        // events sorted by priority (e.g. target doesn't matter if we died)
        if (EventDied(player))
        {
    ...
    string UpdateServer_TRADING(PlayerOLD player)
    {
        // events sorted by priority (e.g. target doesn't matter if we died)
        if (EventDied(player))
        {
    ...
    string UpdateServer_CRAFTING(PlayerOLD player)
    {
        // events sorted by priority (e.g. target doesn't matter if we died)
        if (EventDied(player))
        {
    ...
    string UpdateServer_DEAD(PlayerOLD player)
    {
        // events sorted by priority (e.g. target doesn't matter if we died)
    ...
    public override string UpdateServer(EntityOLD entity)
    {
        PlayerOLD player = (PlayerOLD)entity;
