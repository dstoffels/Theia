...
public class EquipmentItem : UsableItem
{
    ...
    public override bool CanUse(PlayerOLD player, int inventoryIndex)
    {
        return FindEquipableSlotFor(player, inventoryIndex) != -1;
    }

    // can we equip this item into this specific equipment slot?
    public bool CanEquip(PlayerOLD player, int inventoryIndex, int equipmentIndex)
    {
    ...
    int FindEquipableSlotFor(PlayerOLD player, int inventoryIndex)
    {
    ...
    public override void Use(PlayerOLD player, int inventoryIndex)
    {
