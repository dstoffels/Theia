...
[Serializable]
public partial struct SkillOLD
{
    ...
    // constructors
    public SkillOLD(ScriptableSkill data)
    {
    ...
    // events
    public bool CheckSelf(EntityOLD caster, bool checkSkillReady=true)
    {
        return (!checkSkillReady || IsReady()) &&
               data.CheckSelf(caster, level);
    }
    public bool CheckTarget(EntityOLD caster) { return data.CheckTarget(caster); }
    public bool CheckDistance(EntityOLD caster, out Vector3 destination) { return data.CheckDistance(caster, level, out destination); }
    public void Apply(EntityOLD caster) { data.Apply(caster, level); }
    ...
    public string ToolTip(bool showRequirements = false)
    {
        ...
        // addon system hooks
        Utils.InvokeMany(typeof(SkillOLD), this, "ToolTip_", tip);
