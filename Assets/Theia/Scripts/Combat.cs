...
public enum DamageTypeOLD { Normal, Block, Crit }
...
[Serializable] public class UnityEventIntDamageType : UnityEvent<int, DamageTypeOLD> {}
...
#pragma warning restore CS0109 // member does not hide accessible member
    ...
    [Server]
    public virtual void DealDamageAt(EntityOLD victim, int amount, float stunChance=0, float stunTime=0)
    {
        Combat victimCombat = victim.combat;
        int damageDealt = 0;
        DamageTypeOLD damageType = DamageTypeOLD.Normal;
        ...
        if (!victimCombat.invincible)
        {
            ...
            {
                damageType = DamageTypeOLD.Block;
            }
            // deal damage
            else
            {
    ...
    [Client]
    void ShowDamagePopup(int amount, DamageTypeOLD damageType)
    {
        ...
        if (damagePopupPrefab != null)
        {
            ...
            if (damageType == DamageTypeOLD.Normal)
                popup.GetComponentInChildren<TextMeshPro>().text = amount.ToString();
            else if (damageType == DamageTypeOLD.Block)
                popup.GetComponentInChildren<TextMeshPro>().text = "<i>Block!</i>";
            else if (damageType == DamageTypeOLD.Crit)
                popup.GetComponentInChildren<TextMeshPro>().text = amount + " Crit!";
        }
    }

    [ClientRpc]
    public void RpcOnReceivedDamaged(int amount, DamageTypeOLD damageType)
    {
