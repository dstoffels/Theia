...
public class TargetProjectileSkill : DamageSkill
{
    ...
    bool HasRequiredWeaponAndAmmo(EntityOLD caster)
    {
        ...
        if (weaponIndex != -1)
        {
            ...
            WeaponItemOLD itemData = (WeaponItemOLD)caster.equipment.slots[weaponIndex].item.data;
    ...
    void ConsumeRequiredWeaponsAmmo(EntityOLD caster)
    {
        // requires no weapon category?
        // then we can't find weapon and check ammo. just allow it.
        // (monsters have no weapon requirements and don't even have an
    ...
    public override bool CheckSelf(EntityOLD caster, int skillLevel)
    {
        // check base and ammo
    ...
    public override bool CheckTarget(EntityOLD caster)
    {
